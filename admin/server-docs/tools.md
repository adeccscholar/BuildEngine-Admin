...
| `miktex` | 25.12 | managed portable, `when-used` | Isolated portable MiKTeX below `toolsRoot`; compiles the LaTeX tree generated by Doxygen to PDF with `texify`. It does not trigger a second Doxygen run. |

The documentation flow is:

```mermaid
flowchart LR
   D[Doxygen: one run] --> H[HTML]
...
MiKTeX uses the managed root `miktex\25.12-portable`. Portable mode and `--no-additional-roots` prevent BuildEngine from inheriting an unrelated MiKTeX installation from the Windows user profile. One shared runtime preflight initializes mutable runtime data such as format files before independent library PDF jobs are allowed to run in parallel.
...
| Mode | Meaning |
| --- | --- |
| `managed` | BuildEngine downloads, verifies, and installs/extracts the tool below `toolsRoot`. |
...
`miktex` is deliberately `required="when-used"`: it is provisioned only when the effective documentation configuration of at least one library requires PDF output.

This avoids unnecessary tool installation on machines that build HTML documentation only.
...
- changed role/provisioning → update the description,
