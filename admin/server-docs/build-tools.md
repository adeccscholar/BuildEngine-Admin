...
The native extractor also materializes archive links safely after the normal files are present. Relative symbolic-link targets are resolved against the link directory. POSIX archive-absolute targets such as `/usr/...` are interpreted relative to the selected archive root rather than as host filesystem paths. Windows drive roots and UNC targets remain forbidden. Symbolic links are materialized as copied files/directories. TAR hard-link entries without their own payload are deferred and created as Windows hard links to their extracted archive target; missing targets, escaping paths, unsupported target types, and link cycles remain hard failures.
...
Git bootstrap and Bash now use the same pinned official `PortableGit-2.55.0.3-64-bit.7z.exe` payload and the same managed root. The PortableGit self-extractor is used because the raw Git-for-Windows TAR contains MSYS virtual and dangling link entries such as `/dev/fd -> /proc/self/fd`, which are not faithfully representable as ordinary NTFS extraction results. The later logical `bash` entry reuses the existing managed root and probes the supported Git-for-Windows wrapper `bin\\bash.exe`; it does not trigger a second download or installation. Autoconf-style library environments add the wrapper directory, not `usr\\bin`, to their Windows `PATH`.
